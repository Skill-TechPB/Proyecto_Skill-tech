    ...
    %>
    <%
    if (tipou == null || (tipou.equals("4") || tipou.equals("1") || tipou.equals("2"))) {
    response.sendRedirect("index.html");
    return ;}
    %>
    ...
    if(banderitaPoo == 0&& banderitaBD == 1){
    ...
    rs = stmt.executeQuery("select for_url from formulario where for_id=2");
    while(rs.next()){
    ...
    rs3 = stmt.executeQuery("select count(*) as Registros from profesor inner join pro_asi on profesor.pro_id=pro_asi.pro_id where pro_asi.asi_id=1");
    while(rs3.next()){
    reg =rs3.getInt("Registros");
    }
    rs4 = stmt.executeQuery("select pro_nombre, profesor.pro_id from profesor inner join pro_asi on profesor.pro_id=pro_asi.pro_id where pro_asi.asi_id=1");
...
    </div>
    ...
    <div class="row">
           <section>
           <p class="tituform">FORMULARIO DE BASE DE DATOS RELACIONALES</p>
    ...
    <hr> 
    <form method="POST" action="resb" class="form"> 
       <div class="tm">
